...
class VoxelHashMap
{
    public:
        VoxelHashMap(int _local_map_size[3], int _max_size[3], float _initial_value=-1.0, float error_value = NAN)
        {
            ...
            int max_key_count = 1;
            for (int i=0; i<3; i++) max_key_count = max_key_count*std::ceil(max_size[i]/local_map_size[i]);
            std::vector<bool> _keys_allocated(max_key_count, false);
            keys_allocated = _keys_allocated;
        }
        LocalMap empty_map;
        int max_size[3];
        ...
        std::vector<bool> keys_allocated;
        int num_buckets[3];
