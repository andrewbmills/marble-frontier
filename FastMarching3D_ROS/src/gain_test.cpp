...
void NodeManager::CallbackPose(const geometry_msgs::PoseStamped msg)
{
  ...
  pose.position.z = msg.pose.position.z + 1.2;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_map_binary = n.subscribe("octomap_binary", 1, &NodeManager::CallbackOctomapBinary, &node_manager);
  ros::Subscriber sub_map_full = n.subscribe("octomap_full", 1, &NodeManager::CallbackOctomapFull, &node_manager);
  ...
  std::string gainType;
  n.param<std::string>("gain_test_node/gain_type", gainType, "frontier");
  ...
  while (ros::ok())
  {
    ...
    if ((node_manager.map_updated) && (node_manager.pose_updated)) {
      ...
      Gain(node_manager.pose, node_manager.sensor, node_manager.map, node_manager.cloud, gainType);
