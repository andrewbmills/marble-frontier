...
def find_Lookahead_Discrete_2D(path, p_AC, R, gamma_max, Mstar):
    ...
    i_max = path.shape
    i_max = i_max[1] - 1
    # print "The path is {} points long".format(i_max+1)
    intersection = True
    t_hat = -1
    ...
    if intersection:
        ...
        i_cut = i
        # print "There is an intersection at:"
        # print "({}, {}).".format(p_L2[0],p_L2[1])
    else:
        # Calculate lookahead point and its local tangent vector
        # Set lookahead point to the initial path node
        p_L2 = np.array([path[0,0], path[1,0]])
        v_L2 = np.array([path[0,1], path[1,1]]) - p_L2
...
def find_Lookahead_Discrete_3D(path, p_AC, R, gamma_max, Mstar):
    ...
    if intersection:
        ...
        # Calculate lookahead point and its local tangent vector
        # Set lookahead point to the initial path node
        p_L2 = np.array([path[0,0], path[1,0], path[2,0]])
        v_L2 = np.array([path[0,1], path[1,1], path[2,1]]) - p_L2
        v_L2 = v_L2 / np.linalg.norm(v_L2)  # Normalize the vector
        i_cut = 1
        # print "There is no intersection."
